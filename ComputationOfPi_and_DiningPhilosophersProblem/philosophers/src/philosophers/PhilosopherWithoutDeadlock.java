...
public class PhilosopherWithoutDeadlock implements Runnable, Philosopher{
	private Thread t;
	private String name;
	...
	public PhilosopherWithoutDeadlock(String name, Fork leftFork, Fork rightFork){
		this.name = name;
	...
	synchronized public boolean requestEating() throws InterruptedException{
		if(takeRightFork()&&takeLeftFork()){
			return true;
		}else{
			if(tookRightFork){
				System.out.println(name + " puts right fork down " + rightFork.name);
